...
async function findAndDisplayConnectedUsers() {
  try {
    const connectedUsersResponse = await fetch("http://localhost:8080/api/v1/users/connectedUsers");
    ...
    if (connectedUsersResponse.ok) {
      ...
      console.error("Error fetching connected users");
    }
  } catch (error) {
...
async function fetchAndDisplayUserChat() {
  const url = `http://localhost:8080/api/v1/messages/${username}/${selectedUserId}`
...
async function onMessageReceived(payload) {
  ...
  if (selectedUserId && selectedUserId === message.username) {
